...
class Dataset():
    ...
    def real_syn_gen(self, real_ratio=0.0):
    ...
    def get_item(self, item_name):
        ...
        if self.add_noise and rnd_typ != 'real':
            add_real_back_thresh = 0.8
            if rnd_typ == 'render' or self.rng.rand() < add_real_back_thresh:
                rgb = self.rgb_add_noise(rgb)
                rgb_labels = labels.copy()
                msk_dp = dpt_mm > 1e-6
                rgb, dpt_mm = self.add_real_back(rgb, rgb_labels, dpt_mm, msk_dp)
                if self.rng.rand() > add_real_back_thresh:
                    rgb = self.rgb_add_noise(rgb)
